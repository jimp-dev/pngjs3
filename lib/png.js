...
class PNG extends Stream {
  ...
  constructor(options: Props = ({}: any)) {
    super(options);
    const { width, height, fill = false, initGrayscaleData, initPropData } = options;
  ...
  initPropData(shrinkMin: boolean = false, shrinkMax: boolean = false) {
    ...
    if (this.color) {
      ...
      propDataOutput = propData.color(data, shrinkMin, shrinkMax, { width, height, depth });
    }
    else {
      ...
      propDataOutput = propData.grayscale(data, shrinkMin, shrinkMax, { width, height, depth });
    }
  ...
  propData2ImageClamped(clampedDataArray: Uint8ClampedArray, lowerThreshold: number = 0, upperThreshold: number = 1, maxValue: number = 255) {
    if (lowerThreshold < 0) {
